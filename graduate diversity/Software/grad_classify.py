...
def get_gender():
    save_path = 'mcgill/'
    # out_file = '19_mcgill.txt'
    out_file = 'un.txt'
    complete = os.path.join(save_path, out_file)

    filename = 'grad_g.pickle'
                    ...
                    while len(worksheet) == 100:
                        ...
                        for name in worksheet:
                            if name not in name_dict:
                                gender_batch.addItem(name)
                        worksheet = []
                        response_list = gender_batch.classify('f849747198a929e585caca973081c7ca')
                        for name in response_list:
                            name_dict[name.name.strip()] = name.likely_gender
...
def last_dict():
    ...
    # pickle.dump(newest_d,open('grad_g.pickle', 'wb'))

    new_d = pickle.load(open('grad_g.pickle', "rb"))
    # new_d.pop('Biology')
    # new_d['Obaid  Hussen'] = 'male'
    # new_d['Faisal  Tanvir R.'] = 'male'
    new_d = simplejson.loads(simplejson.dumps(new_d, ignore_nan=True))
    pickle.dump(new_d, open('grad_g.pickle', "wb"))
...
def parse():
    ...
    with open(complete) as file:
        for line in file:
            if line not in new_d and line is not None and ':' not in line:
                key = line.strip()
                api_gender = 'https://v2.namsor.com/NamSorAPIv2/api2/json/genderFull/' + key
                gender = requests.get(api_gender, headers={'X-API-KEY': '61f8c3dc68d36e701dd558f668eb4fb9'})
                api_dict = json.loads(gender.text)
                new_d[key] = api_dict['likelyGender']
                pickle.dump(new_d, open('grad_g.pickle', "wb"))
        ...
        for line in file:
            if ':' not in line:
                ...
                if name in gender_dict:
                    applied_dict[name] = [gender_dict[name]]
        print(applied_dict)
        df_20 = pd.DataFrame.from_dict(applied_dict).T
        df_20.to_excel('15_mc_gender.xlsx')
